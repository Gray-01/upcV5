...
<main class="contacts">
  <section class="contacts__section container">
    ...
    <div class="contacts__grid">
      <?php
      // Получаем цвета кнопок из ACF
      $btn_bg = get_field('button_bg');
      $btn_color = get_field('button_color');
      $btn_hover = get_field('button_hover');
      $btn_text = get_field('button_text');
      ?>

      <?php for ($i=1; $i<=5; $i++): ?>
        <?php
          $city = get_field("city_{$i}_name"); // Текстовое поле
          $map  = get_field("city_{$i}_map");  // Текстовое поле с URL
        ?>
        <?php if($city && $map): ?>
          <div class="contacts__card">
            ...
            <a href="<?php echo esc_url($map); ?>"
               target="_blank"
               class="contacts__btn"
               style="background:<?php echo esc_attr($btn_bg); ?>;
